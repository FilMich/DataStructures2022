...
namespace structures
{
    ...
    {
        if (PriorityQueueList<T>::size() < this->capacity_) {
            ...
            {
                PriorityQueueItem<T>* najmensiaPriorita = PriorityQueueList<T>::list_->removeAt(static_cast<int>(PriorityQueueList<T>::size()) - 1);
                this->push(priority, data);
                return najmensiaPriorita;
            }
    ...
    inline int PriorityQueueLimitedSortedArrayList<T>::minPriority()
    {
        ...
        return PriorityQueueList<T>::list_->at(index)->getPriority();
    }

    template<typename T>
    inline bool PriorityQueueLimitedSortedArrayList<T>::trySetCapacity(size_t capacity)
    {
        if (PriorityQueueList<T>::list_->size() < capacity && capacity >= 4)
        {
