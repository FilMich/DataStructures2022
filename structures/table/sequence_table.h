...
namespace structures
{
	...
	protected:
		...
		/// </remarks>
		SequenceTable(List<TableItem<K, T>*>* list); //////////////////////protected konstruktor - je abstraktny
	...
	inline T& SequenceTable<K, T>::find(const K& key)
	{
		...
		if (item == nullptr)
		{
			throw std::out_of_range("SequenceTable<K, T>::find: Key does not exist");
		}
		return item->accessData();
		/*
			T& - odkaz na miesto v pamati, mozeme aj menit
		*/
	}
	...
	inline void SequenceTable<K, T>::insert(const K& key, const T& data)
	{
		...
		if (item != nullptr)
		{
			throw std::logic_error("SequenceTable<K, T>::insert: Key is already present in table!");
		}

		this->list_->add(new TableItem<T, K>(key, data));
	}
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
		...
		if (item == nullptr)
		{
			throw std::out_of_range("SequenceTable<K, T>::remove: Key does not exist in table!");
		}
	...
	{
		TableItem<T, K>* item = this->findTableItem(key);
		return item != nullptr;
	}
