...
namespace structures
{
	...
	{
		for (MultiWayTreeNode<T>* son : *this->children_) {  //post-order
			delete son;
		}
		delete this->children_; //delete obsiahnuteho vrcholu
	...
	{
		return this->children_->at(order);
	}
	...
	inline void MultiWayTreeNode<T>::insertSon(TreeNode<T>* son, int order)
	{
		if (son == nullptr)
		{
			throw std::invalid_argument("MultiWayTreeNode<T>::insertSon: Son cannot be NULL");
		}
		this->children_->insert(dynamic_cast<MultiWayTreeNode<T>*>(son), order);
		son->setParent(this);
	}
	...
	inline TreeNode<T>* MultiWayTreeNode<T>::replaceSon(TreeNode<T>* son, int order)
	{
		if (son == nullptr)
		{
			throw std::invalid_argument("MultiWayTreeNode<T>::replaceSon: Son cannot be NULL");
		}
		MultiWayTreeNode<T>* oldSon = this->children_->at(order);
		oldSon->resetParent();
		this->children_->at(order) = dynamic_cast<MultiWayTreeNode<T>*>(son);
		son->setParent(this);
		return oldSon;
	}
	...
	{
		MultiWayTreeNode<T>* oldSon = this->children_->removeAt(order);
		oldSon->resetParent();
		return oldSon;
	}
