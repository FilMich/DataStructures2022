...
namespace structures
{
	...
	{
		int numberOfProccessedSons = 0;
		auto result = shallowCopy();

		for (int i = 0; numberOfProccessedSons < degree(); i++) {
			auto son = getSon(i);
			if (son != nullptr) {
				result->replaceSon(son->deepCopy(), i);
				numberOfProccessedSons++;
			}
		}
		return result;
	}
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (parent_ != nullptr) {
			...
			throw std::logic_error("Root has no brothers");
		}
	}
	...
	inline void Tree<T>::PreOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
		if (current != nullptr) {
			...
			for (int i = 0; numberOfProccessedSons < current->degree(); i++) {
