...
namespace structures
{
	...
	{
		return this->getParent() != nullptr && this->getParent()->getLeftSon() == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isRightSon()
	{
		return this->getParent() != nullptr && this->getParent()->getRightSon() == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::hasLeftSon()
	{
		return this->getLeftSon() != nullptr;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::hasRightSon()
	{
		return this->getRightSon() != nullptr;
	}
	...
	{
		BinaryTreeNode<T>* cur = dynamic_cast<BinaryTreeNode<T>*>(current);
		if (cur == nullptr)
		{
			return;
		}
		this->populatePath(cur->getLeftSon());
		this->path_->push(cur);
		this->populatePath(cur->getRightSon());
	}
}
